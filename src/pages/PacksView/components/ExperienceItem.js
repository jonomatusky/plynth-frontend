...
const ExperienceItem = ({ experience, isMobile }) => {
  ...
  const updateName = async () => {
    try {
      await updatePack({ id: experience.id, name: name })
    } catch (err) {}
  }

  const { imageUrl } = experience.cards[0]
  // const aspect =
      ...
      <Card>
        <CardActionArea
          ...
          to={
            isMobile
              ? `/p/${experience.id}`
              : `/admin/experiences/${experience.id}/edit/elements`
          }
